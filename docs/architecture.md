...
- `clipboard-win` guard 对应一次显式的 `OpenClipboard` / `CloseClipboard` 生命周期。用于去重、写入验证或粘贴授权的终态序列号，只能在 guard 已释放后读取。
- `arboard` 的单次 `get_*` / `set_*` 方法返回即表示该次系统剪贴板访问结束；外层 Rust 对象本身不代表持续占用 Windows 剪贴板。
- 格式包读取、敏感内容跳过和格式包写入都先关闭显式 guard，再观察终态序列。读回期间序列发生变化时只能重试或失败，不能把不稳定内容授权给直接粘贴或管理员 helper。
- 直接粘贴和一次性管理员 helper 均只接受非零且完全匹配的已验证序列；任何内容回读失败、序列缺失或序列变化都保留副本并降级为手动粘贴。
