  ...
  <section data-testid="library-view" class="library-shell" :aria-label="helpers.t('clipboardManager')" :inert="state.inert">
    ...
    <main class="library-main">
      ...
      <section v-if="state.section !== 'settings'" ref="libraryContent" class="library-content">
        ...
        <div class="manager-list" role="listbox" aria-multiselectable="true" :aria-label="helpers.t('clipboardResults')">
          ...
          <article v-for="(clip, index) in state.historyState === 'ready' ? state.libraryItems : []" :key="clip.id" :data-manager-clip-id="clip.id" class="manager-row" role="option" :tabindex="state.managerSelectedId === clip.id ? 0 : -1" :aria-current="state.managerSelectedId === clip.id ? 'true' : undefined" :aria-selected="helpers.managerClipSelected(clip)" :aria-label="`${clip.title}, ${clip.sourceApp}`" @focus="emit('focusManagerClip', clip.id)" @click="emit('managerRowClick', $event, clip)" @keydown="emit('managerRowKeydown', $event, index, clip.id)">
